...
public class DashboardUpdateService {
    ...
    public ResponseEntity<DashboardDTO> getOrUpdateDashboard(Integer userId) {
        ...
        //➡️ Tìm User xem có tồn tai chưa
        ...
        //➡️ Khi Plan có thì nó sẽ chạy tiếp
        //➡️ Lấy quitedate xem user đã lập kế hoạch ở plan chưa
        Plan plan = planOpt.get();
        LocalDate quitDate = plan.getQuitDate();
        int cigarettesPerDay = plan.getCigarettesPerDay();

        //➡️ Chạy tới đây nó sẽ kiểm tra quitDate có null ko, nếu null thì trả về lỗi và kết thúc.
        if (quitDate == null) {
        ...
        //➡️ Truyền thời gian hiện tại vào findByUserIdAndRecordedDate để lấy dashboard với tời gian là hôm nay
        ...
        //➡️ Nếu hôm nay có dữ liệu thì nó sẽ trả về dashboard lấy được đó thông qua createDTOFromExisting
        ...
        //➡️ Nếu hôm nay chưa có dữ liệu thì nó sẽ tính toán thủ công và  trả về thông qua updateDashboard
        DashboardDTO dto = updateDashboard(userId, plan, trackings, quitDate, cigarettesPerDay);
        return ResponseEntity.ok(dto);
    }


    //➡️ Cập nhật lại Dashboard cho một người dùng nhất định
    ...
    //➡️ Hàm này đươc gọi khi cần update và render lại, hoặc chưa có gì cần tính toán để render
    private DashboardDTO updateDashboard(Integer userId, Plan plan, List<Tracking> trackings,
        ...
        //➡️ Cái này là websocket, là bean của Spring (SimpMessagingTemplate) dùng để gửi message tới các client đang subcribe một topic cụ thể qua WebSocket.
