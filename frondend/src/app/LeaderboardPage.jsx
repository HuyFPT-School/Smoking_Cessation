...
const LeaderboardPage = () => {
  ...
  const [loading, setLoading] = useState(true); // Fetch data when component mounts or timeRange changes
  useEffect(() => {
    ...
    const fetchLeaderboardData = async (range = "weekly") => {
      try {
        ...
        // Only add currentUserId if user exists and has id
        if (user?.id) {
    ...
    // Always fetch data, even if user is not logged in
  ...
  const getFilteredData = () => {
    // Since the API already returns filtered and sorted data based on timeRange,
    // we just need to add ranking and return the data
    return leaderboardData.map((user, index) => ({
      ...user,
      rank: index + 1,
      days: user.consecutiveSmokFreeDays || 0,
    }));
  };
      ...
      {data.length === 0 ? (
        ...
        data.map((user, index) => (
          <React.Fragment key={user.id}>
            ...
            {/* Thêm dấu gạch ngang giữa các user */}
          </React.Fragment>
        ))
      )}
    </div>
              ...
              <div className="user-info" style={{ marginLeft: "16px" }}>
                ...
                >
                  ...
                  <p className="user-days">
                    {currentUserData?.consecutiveSmokFreeDays || 0} smoke-free
                    days
                  </p>
                </div>
              </div>
