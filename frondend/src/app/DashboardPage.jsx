...
import { getApiUrl, getWebSocketUrl } from "../config/apiConfig";
...
const DashboardPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const dashboardResponse = await fetch(
          getApiUrl(`/api/dashboard/${userId}`)
        );
        if (!dashboardResponse.ok) {
          throw new Error(
            `Failed to fetch dashboard data: ${await dashboardResponse.text()}`
          );
        }
        ...
        const historyResponse = await fetch(
          getApiUrl(`/api/dashboard/history/${userId}`)
        );
        if (!historyResponse.ok) {
          throw new Error(
            `Failed to fetch history data: ${await historyResponse.text()}`
          );
        }
    ...
    const client = new Client({
      webSocketFactory: () => new SockJS(getWebSocketUrl()), // địa chỉ WebSocket
          ...
          try {
            ...
            if (historyResponse.ok) {
              ...
              const chartData = historyJson.map((item) => ({
                ...
                averageCravingSatisfaction:
                  item.averageCravingSatisfaction ?? 0,
              }));
  ...
  const satisfactionChartConfig = {
    ...
    point: {
      // Hiển thị các điểm tròn tại mỗi ngày
      size: 4,
      shape: "circle",
    },
    ...
    tooltip: {
      ...
      formatter: (datum) => ({
        ...
        value:
          datum.averageCravingSatisfaction != null
  ...
  return (
    <div className="Dashboard-Backgroup">
      ...
      <p style={{ color: "#595959", marginBottom: "24px", fontSize: "14px" }}>
        You've accumulated {dashboardData.daysSmokeFree || 0} smoke-free days.
        Keep going!
      </p>

      <Row
        gutter={[{ xs: 8, sm: 16, md: 24 }, 24]}
        className="dashboard-row-spacing"
      >
        <Col xs={24} sm={12} md={6}>
          <div className="dashboard-card">
            ...
            <p className="dashboard-card-value">
              {dashboardData.daysSmokeFree || 0}
            </p>
            <p className="dashboard-card-subtext">
              Total smoke-free days accumulated!
            </p>
          </div>
        </Col>
        <Col xs={24} sm={12} md={6}>
          <div className="dashboard-card">
            ...
            <p className="dashboard-card-subtext">
              Based on ${dashboardData.cigarettesPerDay * 0.04}/day
            </p>
          </div>
        </Col>
        <Col xs={24} sm={12} md={6}>
          <div className="dashboard-card">
            ...
            <p className="dashboard-card-value">
              {dashboardData.cigarettesAvoided || 0}
            </p>
            <p className="dashboard-card-subtext">
              Based on {dashboardData.cigarettesPerDay}/day
            </p>
          </div>
        </Col>
        <Col xs={24} sm={12} md={6}>
          <div className="dashboard-card">
            ...
            <p className="dashboard-card-value">
              {dashboardData.nextMilestone || "Completed"}
            </p>
            <p className="dashboard-card-subtext">
              {dashboardData.remainingDaysToMilestone || 0} days to go
            </p>
          </div>
        </Col>
      </Row>
                  ...
                  <div className="stat-details">
                    <div>
                      ...
                      <span style={{ fontWeight: "bold" }}>
                        {dashboardData.todayCigarettes || 0}
                      </span>
                    </div>
                    <div>
                      <span className="stat-label">Cravings:</span>{" "}
                      <span style={{ fontWeight: "bold" }}>
                        {dashboardData.todayCravings || 0}
                      </span>
                    </div>
                  </div>
                </Col>
                <Col xs={24} sm={12} md={12} className="stat-item">
                  ...
                  <div className="stat-details">
                    <div>
                      <span className="stat-label">Cigarettes:</span>{" "}
                      <span style={{ fontWeight: "bold" }}>
                        {dashboardData.yesterdayCigarettes || 0}
                      </span>
                    </div>
                    <div>
                      <span className="stat-label">Cravings:</span>{" "}
                      <span style={{ fontWeight: "bold" }}>
                        {dashboardData.yesterdayCravings || 0}
                      </span>
                    </div>
                  </div>
                </Col>
                ...
                <Col span={24} className="stat-item">
                  ...
                  <div className="stat-details">
                    <div>
                      ...
                      <span style={{ fontWeight: "bold" }}>
                        {dashboardData.last7DaysCigarettes || 0}
                      </span>
                    </div>
                    <div>
                      <span className="stat-label">Total cravings:</span>{" "}
                      <span style={{ fontWeight: "bold" }}>
                        {dashboardData.last7DaysCravings || 0}
                      </span>
                    </div>
                    <div>
                      <span className="stat-label">Resistance rate:</span>{" "}
                      <span style={{ fontWeight: "bold" }}>
                        {dashboardData.resistanceRate || 0}%
                      </span>
                    </div>
                  </div>
                </Col>
              ...
              options={[
                {
                  label: (
                    <span
                      style={{
                        fontWeight:
                          chartMetric === "cigarettes" ? "bold" : "normal",
                      }}
                    >
                      🚬 Smoking chart
                    </span>
